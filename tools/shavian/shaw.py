...
def alpha_split(str):
  ...
  for i in range(len(str)):
    alpha = str[i].isalpha() or \
	"'.".find(str[i])+1 and i > 0 and i < len(str)-1 and \
	str[i-1].isalpha() and str[i+1].isalpha()
    if alpha == prev:
      list[-1] += str[i]
    else:
      list += str[i]
...
def lookup (word, pos):
  ...
  low = word.lower()
  pos = "_" + pos
  for look in [ low+pos, low+pos[:3], word, word+".",
	low, low+".", low+"_NN", low+"_NNS",
	word[0].upper()+word[1:], word[0].upper()+low[1:], word.upper() ]:
    if look in dict:
      ret = dict[look]
      if ret.find(".")+1:
      ...
      if (word[0].isupper() or look[0].isupper()) \
	   and (look[-1] != "." or word != whole) and ret[-1] > "z":
        ret = "·" + ret
      break
  return ret
...
def suffix_split (inp, pos, adj):
  ...
  for split in range(max(long-9,2),long):
    suff = "$"+low[split:]
    if not suff in dict or low[split-1:split+1] in ["ee","ss"]: continue
    if low[split:] == "ess" and low[split-2] == low[split-1]: continue
    if low[split:] == "ry" and "aeiouf".find(low[split-1])+1: continue
    if low[split:] == "r" and "aeiou".find(low[split-1])+1: continue
    if low[split:] == "n" and "eio".find(low[split-1])+1 \
    ...
    for pess in range(2):
      if (pess):
        ...
        if low[split-1] == low[split-2] and not "sh".find(low[split-1])+1:
          word = inp[:split-1]
        elif ( "cghlsuvz".find(low[split-1])+1 or low[split] == 'e' or \
		"aeiousy".find(low[split-2])+1 ) and \
		("cg".find(low[split-1])<0 or "aou".find(low[split])<0):
          word = inp[:split] + "e"
        else: continue
      ...
      if mid == "𐑦𐑩": mid = "𐑾"
      if mid == "𐑦𐑼": mid = "𐑽"
      if mid == "𐑤𐑤" and len(suff) < 3: mid = "𐑤"
...
cont = { "'s":"'𐑟","'d":"'𐑛","'ll":"'𐑤","'m":"'𐑥","'re":"'𐑼","'ve":"'𐑝","n't":"𐑯'𐑑" }
dict = { "ai":"𐑱","ca":"𐑒𐑭.","gim":"𐑜𐑦𐑥","lem":"𐑤𐑧𐑥","na":"𐑯𐑩","ta":"𐑑𐑩.","wo":"𐑢𐑴" }
...
#  print (token)
  ...
  if low in cont:
    apos = cont[low]
    if low == "'s":
      if  "𐑐𐑑𐑒𐑓𐑔".find(out[-1])+1: apos = "'𐑕"       
      if "𐑕𐑖𐑗𐑟𐑠𐑡".find(out[-1])+1: apos = "'𐑩𐑟"
  ...
  if token[0][0].isalnum():
    out += " "
  if prev[0] == "can" and low == "not" or \
     prev[0] == "got" and low == "ta" or \
     prev[0] == "lem" and low == "me" or \
     prev[0] == "gim" and low == "me" or \
     prev[0] == "gon" and low == "na" or \
     prev[0] == "wan" and low == "na":
    out = out[:-2]
    token = (low,token[1])
